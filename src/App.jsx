...
export default function App(){
  ...
  // Carregar dados salvos ao iniciar (localStorage — padrão do browser)
  useEffect(()=>{
    try {
      const s = localStorage.getItem("bra2026:jogos");
      if(s) setJogos(JSON.parse(s));
    } catch(_){}
    try {
      const s = localStorage.getItem("bra2026:servicos");
      if(s) setServicos(JSON.parse(s));
    } catch(_){}
    try {
      const s = localStorage.getItem("bra2026:darkMode");
      if(s !== null) setDarkMode(JSON.parse(s));
    } catch(_){}
  },[]);
  ...
  const setJogosP = (fn) => {
    setJogos(prev => {
      ...
      try { localStorage.setItem("bra2026:jogos", JSON.stringify(next)); } catch(_){}
      return next;
    });
  };
  ...
  const setServicosP = (fn) => {
    setServicos(prev => {
      const next = typeof fn === "function" ? fn(prev) : fn;
      try { localStorage.setItem("bra2026:servicos", JSON.stringify(next)); } catch(_){}
      return next;
    });
  };
  ...
  const setDarkModeP = (v) => {
    ...
    setDarkMode(next);
    try { localStorage.setItem("bra2026:darkMode", JSON.stringify(next)); } catch(_){}
  };

  const T = darkMode ? DARK : LIGHT;

  const varCalc = useMemo(() => {
    const allJogos = jogos.filter(j => j.mandante !== "A definir");
