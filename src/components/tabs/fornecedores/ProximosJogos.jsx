...
const COR_TEMPORAL = { passado:"#94a3b8", hoje:"#ef4444", urgente:"#f59e0b", semana:"#3b82f6", futuro:"#10b981", sem_data:"#64748b" };
const LABEL_TEMPORAL = { passado:"Passado", hoje:"Hoje", urgente:"Em 3 dias", semana:"Esta semana", futuro:"Futuro", sem_data:"Sem data" };
...
function calcularCobertura(jogo, tabelas, fornecedores, campeonatos) {
  ...
  const itens = camp.itens?.length ? camp.itens.filter(i=>i.ativo!==false) : [];
  if (!itens.length) return { status:"sem_itens", fornecedores:[], custoTotal:0, custoMin:0 };
  ...
  fornecedores.forEach(f => {
    ...
    if (cobertos > 0) {
      fornComCobertura.push({
        fornecedorId: f.id,
        apelido: f.apelido,
        cobertos,
        total: itens.length,
        custo: total,
        coberturaPct: Math.round((cobertos/itens.length)*100),
      });
    }
  });

  if (!fornComCobertura.length) return { status:"sem_cobertura", fornecedores:[], custoTotal:0, custoMin:0 };

  const coberturasCompletas = fornComCobertura.filter(f=>f.coberturaPct===100);
  const custos = fornComCobertura.map(f=>f.custo).filter(x=>x>0);
  const custoMin = custos.length ? Math.min(...custos) : 0;
  const custoMax = custos.length ? Math.max(...custos) : 0;

  return {
    status: coberturasCompletas.length>0 ? "completa" : "parcial",
    fornecedores: fornComCobertura,
    custoMin,
    custoMax,
    custoMedio: custos.length ? custos.reduce((a,b)=>a+b,0)/custos.length : 0,
  };
}

// ── Semáforo visual ──────────────────────────────────────────────────────
function CoberturaIndicator({ cob, T }) {
  if (!cob) return <span style={{color:T.textSm,fontSize:11}}>—</span>;
  const config = {
    completa:      { color:T.brand||"#10b981", label:"Cobertura total",    icon:<CheckCircle2 size={12}/> },
    parcial:       { color:T.warning||"#f59e0b", label:"Cobertura parcial", icon:<AlertCircle size={12}/> },
    sem_cobertura: { color:T.danger||"#ef4444", label:"Sem cobertura",      icon:<AlertTriangle size={12}/> },
    sem_itens:     { color:"#64748b",            label:"Sem itens",          icon:<AlertTriangle size={12}/> },
  };
  const cfg = config[cob.status] || config.sem_cobertura;
  return (
    <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"3px 9px",borderRadius:RADIUS.pill,background:`${cfg.color}18`,color:cfg.color,fontSize:11,fontWeight:700}}>
      {cfg.icon}{cfg.label}
    </span>
  );
}
...
function JogoModal({ jogo, campeonatos, cidades, onSave, onClose, T }) {
  ...
  const camposPadrao = useMemo(()=>{
    ...
    return { campeonatoId:camp?.id||"", cidadeId:camp?.cidadeIds?.[0]||"", categoria:camp?.categorias?.[0]?.codigo||"", rodada:1, data:"", mandante:"", visitante:"", estadio:"" };
  },[campeonatos]);
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",backdropFilter:"blur(4px)",zIndex:130,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:T.surface||T.card,borderRadius:RADIUS.xl,padding:28,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto",border:`1px solid ${T.border}`,boxShadow:T.shadow}}>
        ...
        <p style={{margin:"0 0 18px",fontSize:12,color:T.textMd}}>Cadastro manual do calendário de jogos.</p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 12px"}}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <select value={form.categoria} onChange={e=>set("categoria",e.target.value)} style={IS}>
              <option value="">— Selecione —</option>
              {(camp?.categorias||[]).map(c=><option key={c.codigo} value={c.codigo}>{c.codigo}</option>)}
            </select>
          </div>
          <div style={{marginBottom:12}}>
            <label style={lbl}>Rodada</label>
          ...
          <div style={{marginBottom:12,gridColumn:"1 / -1"}}>
            <label style={lbl}>Estádio (opcional)</label>
...
export default function ProximosJogos({
  ...
  // Cobertura por jogo
  const coberturaByJogo = useMemo(()=>{
    const map = {};
    jogosForn.forEach(j => {
      map[j.id] = calcularCobertura(j, tabelas, fornecedores, campeonatos);
    });
    return map;
  }, [jogosForn, tabelas, fornecedores, campeonatos]);
  ...
  const jogosFiltrados = useMemo(()=>{
    ...
    if (busca.trim()) {
      ...
      lista = lista.filter(j=>(j.mandante||"").toLowerCase().includes(q)||(j.visitante||"").toLowerCase().includes(q)||(j.estadio||"").toLowerCase().includes(q));
    }
    lista = lista.map(j=>({...j, _temporal:classificarTemporal(j,agora)}))
      .filter(j=>{
    ...
    if (filtroCobertura!=="todas") {
      lista = lista.filter(j=>(coberturaByJogo[j.id]?.status||"sem_cobertura")===filtroCobertura);
    }
    return lista.sort((a,b)=>(a.data||"").localeCompare(b.data||""));
  }, [jogosForn,filtroCampeonato,filtroCidade,filtroCategoria,busca,filtroTemporal,filtroCobertura,agora,coberturaByJogo]);

  const kpis = useMemo(()=>{
    const escopo = filtrarJogos(jogosForn,{campeonatoId:filtroCampeonato!=="todos"?filtroCampeonato:undefined})
      .map(j=>({...j,_t:classificarTemporal(j,agora)}));
    const futuros = escopo.filter(j=>["hoje","urgente","semana","futuro"].includes(j._t));
    const semCobertura = futuros.filter(j=>(coberturaByJogo[j.id]?.status||"sem_cobertura")==="sem_cobertura").length;
    const custoProjetado = futuros.reduce((s,j)=>{
      const cob = coberturaByJogo[j.id];
      return s + (cob?.custoMin||0);
    },0);
    return {
      total:    escopo.length,
  ...
  },[jogosForn,filtroCampeonato,agora,coberturaByJogo]);

  const cotacoesPorJogo = useMemo(()=>{
    const map = {};
    (cotacoes||[]).forEach(c=>{ if(!c.jogoId) return; map[c.jogoId]=(map[c.jogoId]||0)+1; });
    return map;
  },[cotacoes]);

  const cidadesDisponiveis = useMemo(()=>{
    if (filtroCampeonato==="todos") return cidades;
    return (campById[filtroCampeonato]?.cidadeIds||[]).map(id=>cidadeById[id]).filter(Boolean);
  ...
  return (
    <>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14,marginBottom:20}}>
        ...
        <KPI label="Sem cobertura" value={String(kpis.semCobertura)} sub="Jogos futuros sem fornecedor" color={kpis.semCobertura>0?(T.danger||"#ef4444"):"#64748b"} T={T}/>
        <KPI label="Custo projetado" value={fmt(kpis.custoProjetado)} sub="Menor preço disponível por jogo futuro" color={T.warning||"#f59e0b"} T={T}/>
      </div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <div style={{position:"relative"}}>
            ...
            <input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar por time ou estádio..." style={{...iSty(T),width:240,padding:"8px 12px 8px 34px"}}/>
          </div>
          ...
          <div style={{width:1,height:24,background:T.border}}/>
          <Chip active={filtroCobertura==="todas"} onClick={()=>setFiltroCobertura("todas")} T={T}>Todas</Chip>
          <Chip active={filtroCobertura==="completa"} onClick={()=>setFiltroCobertura("completa")} T={T} color={T.brand||"#10b981"}>Cobertura total</Chip>
          <Chip active={filtroCobertura==="parcial"} onClick={()=>setFiltroCobertura("parcial")} T={T} color={T.warning||"#f59e0b"}>Parcial</Chip>
          <Chip active={filtroCobertura==="sem_cobertura"} onClick={()=>setFiltroCobertura("sem_cobertura")} T={T} color={T.danger||"#ef4444"}>Sem cobertura</Chip>
          <div style={{width:1,height:24,background:T.border}}/>
          <select value={filtroCidade} onChange={e=>setFiltroCidade(e.target.value)} style={{...iSty(T),width:150}}>
            <option value="todas">Todas cidades</option>
            {cidadesDisponiveis.map(c=><option key={c.id} value={c.id}>{c.nome}/{c.uf}</option>)}
          </select>
          <select value={filtroCategoria} onChange={e=>setFiltroCategoria(e.target.value)} style={{...iSty(T),width:120}}>
        ...
        {jogosFiltrados.length===0?(
          ...
          <div style={TS.wrap}>
            <table style={{...TS.table,minWidth:1100}}>
              <thead>
                <tr style={TS.thead}>
                  {["Tempo","Data","Rnd","Cidade","Cat.","Jogo","Cobertura","Custo mín.","Cot.",""].map(h=>(
                    <th key={h} style={{...TS.th,...TS.thLeft,...(["Custo mín."].includes(h)?{textAlign:"right"}:{})}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={j.id} style={TS.tr}>
                      ...
                      <td style={{...TS.td,fontSize:12}}>
                        <span style={{display:"inline-flex",alignItems:"center",gap:5}}>
                          <MapPin size={11} color={T.textSm}/>{cid?`${cid.nome}/${cid.uf}`:"—"}
                        </span>
                      </td>
                      <td style={TS.td}>
                      ...
                      <td style={TS.td}>
                        <CoberturaIndicator cob={cob} T={T}/>
                        {cob?.fornecedores?.length>0&&(
                          <div style={{fontSize:10,color:T.textSm,marginTop:3}}>
                            {cob.fornecedores.map(f=>f.apelido).join(", ")}
                          </div>
                        )}
                      </td>
                      <td style={{...TS.td,textAlign:"right",fontFamily:"'JetBrains Mono',ui-monospace,monospace",fontSize:12}}>
                        {cob?.custoMin>0?(
                          <div>
                            <span style={{fontWeight:700,color:T.brand||"#10b981"}}>{fmt(cob.custoMin)}</span>
                            {cob.custoMax>cob.custoMin&&<div style={{fontSize:10,color:T.textSm}}>até {fmt(cob.custoMax)}</div>}
                          </div>
                        ):<span style={{color:T.textSm}}>—</span>}
                      </td>
                      <td style={TS.td}>
                        {numCot>0
                          ?<Badge T={T} color={T.brand||"#10b981"} size="sm">{numCot}</Badge>
      ...
      {showModal&&(
        <JogoModal jogo={editing} campeonatos={campeonatos} cidades={cidades} onSave={salvar} onClose={()=>{setShowModal(false);setEditing(null);}} T={T}/>
      )}
    </>
  );
}
